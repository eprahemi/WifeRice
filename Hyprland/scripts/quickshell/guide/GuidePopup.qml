            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                            ListElement { version: "v1.7.17"; title: "Battery Audio Fix + One-Time zshrc"; desc: "battery_fetch.sh now uses pw-play/paplay + nohup, zshrc deploys once only"; icon: "󰁹"; clr: "green"; detail: "Three changes: (1) Added pw-play and paplay as primary audio players in battery_fetch.sh — mpg123/ffplay fail silently when spawned from Quickshell's Process sandbox. (2) Added nohup + explicit /dev/null redirect to fully detach audio playback from the shell process tree. (3) install.sh now deploys .zshrc only once (flag file at ~/.cache/qs_zshrc_deployed), so future updates never overwrite user customisations. Removed opencode PATH from default .zshrc." }
